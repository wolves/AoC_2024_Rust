pub fn process(input: &str) -> miette::Result<String> {
    ...
    let first_password = next_password(input)?;
    let result = next_password(&first_password);
...
mod tests {
    ...
    #[test]
    fn test_multiple_next_passwords() -> miette::Result<()> {
        let input = "abcdefgh";
        let first = next_password(input)?;
        let second = next_password(&first)?;
        assert_eq!(first, "abcdffaa");
        assert_eq!(second, "abcdffbb");
        Ok(())
    }
