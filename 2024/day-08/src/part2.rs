        ...
        .flat_map(|chunk| {
            chunk
                ...
                .flat_map(|sats| {
                    ...
                    let first_results: Vec<_> = successors(Some(sats[0].0), |pos| {
                        let new_pos = pos + diff;
                        if x_bound.contains(&pos.x) && y_bound.contains(&pos.y) {
                            Some(new_pos)
                        } else {
                            None
                        }
                    ...
                    let second_results: Vec<_> = successors(Some(sats[1].0), |pos| {
                        let new_pos = pos - diff;
                        if x_bound.contains(&pos.x) && y_bound.contains(&pos.y) {
                            Some(new_pos)
                        } else {
